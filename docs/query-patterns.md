...
How selectors own query shape, how to keep a list endpoint at a fixed
query count regardless of row count, and how to add a query-count test
to a new endpoint.
...
the same queryset again in the view is the other pattern found and
removed here (`jobs.views.list_jobs` used to build the queryset via
...
any of the other six in the table below:
...
The seven endpoints this repo pins:
...
(`GET /orgs/<slug>/jobs/` isn't one of the seven listed above but is
covered too — `jobs/tests/test_query_counts.py`, 6 queries, the extra
one being the `steps` prefetch.)
...
| `AuditLog`                         | `(-id,)` (ddia#18 — UUIDv7 `id` is monotonic and already unique; `created_at` isn't, for two causally-ordered writes racing across app servers) | `(organization, created_at)` only, before this fix | **gap — fixed**: added `(organization, id)` in `keel/audit/migrations/0002_audit_log_organization_id_index.py`                                                                  |
