...
runs in, unlike a *browser's* `fetch()` — returns the real 3xx response
...
*after* Google: it redirects back to whatever `redirect_uri` was
...
bags) is *not* touched here. It's tagged fold-into-phase-11 in the
