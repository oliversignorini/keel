...
Driven by ``keel.core.authz.registered_global_viewsets()`` rather than a
router walk, so a ``GlobalViewSet`` registered on any router — or on none
at all — still gets its justification printed here (PRD §4 invariant 7).
"""
