...
| 6   | Every Ninja resource declares `organization_scoped = True` + `test_factory`, or `organization_scoped = False` + `GLOBAL_JUSTIFICATION`. Cross-org access on a scoped resource returns 404, not 403.                                                       | `OrgScopedResource.__init_subclass__` (`keel/core/authz.py`) fails at import if neither is present; `keel/organizations/tests/test_meta_router_wiring.py` walks the router; `test_ninja_tenant_isolation.py` asserts 404 | `cd apps/api && uv run pytest keel/organizations/tests/test_meta_router_wiring.py keel/organizations/tests/test_ninja_tenant_isolation.py` |
| 7   | Every mutating service is `@audited("action.name")` or `@not_audited(reason=...)`, names its actor parameter `actor` (the decorator reads `kwargs["actor"]`), and returns the row it acted on so the audit row has a target — including deletes. Coverage is gated per directory (`[tool.keel.coverage]` in `apps/api/pyproject.toml`), not by one global number.                                                       | `keel/audit` meta-test fails on a service decorated with neither, and on an `@audited` service with no `actor` parameter (`ACTORLESS_AUDITED_SERVICES` is the documented escape hatch); `scripts/check_coverage.py` reads `coverage.json` after pytest                                                                                         | `cd apps/api && uv run pytest && python3 ../../scripts/check_coverage.py`                                                                  |
...
- `packages/api-client/src/generated` is never hand-edited. It is produced
  ...
  from the Ninja and allauth specs). CI's `api-client-generation` and
...
- **ADR 0001 replaced DRF with Django Ninja.** The commands
  ...
  OpenAPI document Ninja generates natively (no drf-spectacular). See
  `docs/adr/0001-django-ninja-over-drf.md` for why the migration happened.
